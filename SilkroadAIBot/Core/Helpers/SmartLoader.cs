...
namespace SilkroadAIBot.Core.Helpers
{
    public static class SmartLoader
    {
        ...
        public static bool LaunchAndPatch(string clientPath, string originalIp, string redirectUrl, int locale = 22)
        {
            ...
            string commandLine = $"\"{clientPath}\" 0 /{locale} 0 0";
            ...
            if (CreateProcess(null, commandLine, IntPtr.Zero, IntPtr.Zero, false, CREATE_SUSPENDED, IntPtr.Zero, workingDir, ref si, out pi))
            {
                try
                {
                    LogService.Info($"[Loader] Process created (PID: {pi.dwProcessId}). Initiating Smart Delayed Memory Patcher...");

                    // Resume the thread immediately so the client can unpack the IP into memory
                    ResumeThread(pi.hThread);

                    // Start an asynchronous polling task to hunt down the IP as soon as it decrypts
                    System.Threading.Tasks.Task.Run(async () =>
                    {
                        LogService.Info("[Scanner] Waiting for client to unpack IP into memory...");
                        int maxAttempts = 25; // 5 seconds total (25 * 200ms)
                        
                        for (int i = 0; i < maxAttempts; i++)
                        {
                            await System.Threading.Tasks.Task.Delay(200); // Wait 200ms before checking
                            
                            int foundCount = UniversalPatch(pi, originalIp, redirectUrl);
                            
                            if (foundCount > 0)
                            {
                                LogService.Info($"[Scanner] SUCCESS! Patched IP {foundCount} times at attempt {i + 1}. Client is safely redirected!");
                        ...
                        LogService.Error("[Scanner] Timeout: Failed to find IP in memory after 5 seconds. Client may have connected to real server.");
                        CloseHandle(pi.hThread);
                        CloseHandle(pi.hProcess);
                    });
        ...
        private static int UniversalPatch(PROCESS_INFORMATION pi, string originalIp, string redirectUrl)
        {
            int totalFound = 0;
