                ...
                while (true)
                {
                    ...
                    if (packets != null)
                    {
                        foreach(var packet in packets)
                        {
                            ...
                            SilkroadAIBot.Core.Helpers.PacketLogger.Log("C->S", packet, true);
             ...
             try 
            {
                ...
                while (true)
                {
                    ...
                    if (packet != null)
                    {
                        ...
                        SilkroadAIBot.Core.Helpers.PacketLogger.Log("S->C", packet, false);
                        ...
                        {
                            packet.Lock();
                            // Create a copy of the packet to read from it without breaking the original stream
                            ...
                            if (result == 0x01) // Login Success
                            {
                                ...
                                SilkroadAIBot.Core.Helpers.LogService.Info($"[Gateway] Login Success! Server wants client to go to Agent: {realAgentIp}:{realAgentPort}");
                                
                                // 1. Define our new local Agent Port
                                int localAgentPort = 15778; 
                                
                                // 2. Start a new ProxyManager for the Agent Server dynamically
                                Task.Run(() => {
                                    ...
                                    SilkroadAIBot.Core.Helpers.LogService.Info($"[Agent Proxy] Started listening on 127.0.0.1:{localAgentPort} -> Forwarding to {realAgentIp}:{realAgentPort}");
                                });
                                
                                // 3. Modify the packet sent to the client to point to our local Agent Proxy
                                Packet modifiedPacket = new Packet(0xA102, true);
                                modifiedPacket.WriteByte(0x01);
                                modifiedPacket.WriteUInt(sessionId);
                                modifiedPacket.WriteAscii("127.0.0.1"); // Force client to connect to us
                                modifiedPacket.WriteUShort((ushort)localAgentPort);
                                
                                // Send the modified packet to the client INSTEAD of the original
                                _clientSecurity.Send(modifiedPacket);
                                SilkroadAIBot.Core.Helpers.LogService.Info("[Gateway] Sent spoofed Agent IP (127.0.0.1:15778) to Client.");
                                continue; // Skip forwarding the original packet
                            }
                        }
...
namespace SilkroadAIBot.Proxy
{
    public class ProxyManager
    {
        ...
        private SilkroadAIBot.Networking.ClientlessConnection _serverConnection = null!;
        ...
        public async Task<bool> StartAsync()
        {
            try
            {
                Console.WriteLine($"[ProxyManager] Connecting to Real Server {_targetGatewayIP}:{_targetGatewayPort}...");
                _serverConnection = new SilkroadAIBot.Networking.ClientlessConnection();
                await _serverConnection.ConnectAsync(_targetGatewayIP, _targetGatewayPort);

                // Start the receive loop immediately so it can process the 0x5000 handshake packet
                // that the server sends upon connection, instead of waiting forever.
                _ = _serverConnection.ProcessAsync();

                await _serverConnection.PerformHandshakeAsync();
                Console.WriteLine("[ProxyManager] Handshake with Real Server completed successfully.");
            ...
            {
                Console.WriteLine($"[ProxyManager] Failed to start and connect to server: {ex.Message}");
                SilkroadAIBot.Core.Helpers.LogService.Error($"[ProxyManager] Failed to start and connect to server: {ex.Message}");
                return false;
            }
        }
        ...
        {
            SilkroadAIBot.Core.Helpers.LogService.Info("[Proxy] A client has just knocked on the door! (Connection Attempt)");
            Console.WriteLine("[ProxyManager] New Client Connection.");
            
            // Pass the already connected _serverConnection to the ProxyContext
            var context = new ProxyContext(client, _worldState, _serverConnection);
            if (_dataManager != null) context.Initialize(_dataManager);
            _sessions.Add(context);
            
            // Start the proxy session loop using the existing connection
            _ = Task.Run(() => context.StartProxySessionAsync());
        }
    }
}
#endregion
