...
namespace SilkroadAIBot.UI
{
    public partial class MainForm : Form
    {
        ...
        // Views
        private Panel _viewSetup = null!, _viewDashboard = null!, _viewConfig = null!, _viewLogs = null!, _viewPackets = null!;
        private RichTextBox _rtbLogs = null!;
        
        // Packets View Controls
        private ListView _lvPackets = null!;
        private CheckBox _chkLogCS = null!, _chkLogSC = null!, _chkIgnoreHandshakes = null!;
        ...
        private void InitializeLayout()
        {
            ...
            _viewPackets = CreateViewPanel("Packets");
            ...
            SetupView5_Packets(_viewPackets);
            ...
            AddNavButton("Packets", _viewPackets);
        ...
        // VIEW 5: PACKETS
        // ==========================================
        private void SetupView5_Packets(Panel page)
        {
            Panel pnlTop = new Panel { Dock = DockStyle.Top, Height = 45, BackColor = UIPalette.BackLight, Padding = new Padding(5) };
            page.Controls.Add(pnlTop);

            _chkLogCS = new CheckBox { Text = "Log C->S", Location = new Point(10, 12), Checked = true, AutoSize = true, ForeColor = UIPalette.TextMain };
            _chkLogSC = new CheckBox { Text = "Log S->C", Location = new Point(100, 12), Checked = true, AutoSize = true, ForeColor = UIPalette.TextMain };
            _chkIgnoreHandshakes = new CheckBox { Text = "Ignore Handshakes (0x5000/0x9000)", Location = new Point(190, 12), Checked = true, AutoSize = true, ForeColor = UIPalette.TextMain };
            
            Button btnClear = CreateFlatButton("Clear", new Point(420, 8), new Size(80, 28), UIPalette.AccentOrange);
            btnClear.Click += (s, e) => _lvPackets.Items.Clear();

            pnlTop.Controls.AddRange(new Control[] { _chkLogCS, _chkLogSC, _chkIgnoreHandshakes, btnClear });

            _lvPackets = new ListView
            {
                Dock = DockStyle.Fill,
                View = View.Details,
                FullRowSelect = true,
            ...
            _lvPackets.Columns.Add("Time", 80);
            _lvPackets.Columns.Add("Dir", 50);
            _lvPackets.Columns.Add("Opcode", 70);
            _lvPackets.Columns.Add("Len", 50);
            _lvPackets.Columns.Add("Data (Hex)", 600);
            
            page.Controls.Add(_lvPackets);
            _lvPackets.BringToFront();

            // Hook Event
            PacketLogger.OnPacketLogged += HandlePacketLogged;
        }

        private void HandlePacketLogged(string direction, Packet packet, bool isCS)
        {
            SafeInvoke(() => 
            {
                if (isCS && !_chkLogCS.Checked) return;
                if (!isCS && !_chkLogSC.Checked) return;
                
                if (_chkIgnoreHandshakes.Checked)
                {
                    if (packet.Opcode == 0x5000 || packet.Opcode == 0x5001 || packet.Opcode == 0x9000 || packet.Opcode == 0x2001) return;
                }

                if (_lvPackets.Items.Count > 1000)
                {
                    _lvPackets.Items.RemoveAt(0); // keep memory from flying away
                }

                var item = new ListViewItem(DateTime.Now.ToString("HH:mm:ss"));
                item.SubItems.Add(direction);
                item.SubItems.Add($"0x{packet.Opcode:X4}");
                
                try 
                {
                    byte[] rawPayload = packet.GetBytes();
                    item.SubItems.Add(rawPayload.Length.ToString());
                    item.SubItems.Add(BitConverter.ToString(rawPayload).Replace("-", " "));
                }
                catch { item.SubItems.Add("0"); item.SubItems.Add("ERR"); }

                item.ForeColor = isCS ? UIPalette.AccentBlue : UIPalette.AccentGreen;
                if (packet.Opcode == 0x5000 || packet.Opcode == 0x9000) item.ForeColor = UIPalette.Error;
                
                _lvPackets.Items.Add(item);
                _lvPackets.EnsureVisible(_lvPackets.Items.Count - 1);
            });
        }
        ...
        private async Task StartGameSequence()
        {
            ...
            if (_chkProxyMode.Checked)
            {
                ...
                try
                {
                    ...
                    int localProxyPort = targetPort; // Dynamically match the real server's port (e.g. 15779)
                    
                    // Force the overwrite in json configuration in case the user has the old 15777 cached
                    SilkroadAIBot.Core.Configuration.ConfigManager.Config.ProxyPort = localProxyPort;
                    SilkroadAIBot.Core.Configuration.ConfigManager.Save();
                    ...
                    bool connected = await _proxy.StartAsync();

                    if (connected)
                    {
                        LogService.Info($"[Proxy] Connected to Server. Listening for client on port {localProxyPort}...");
                        LogService.Info($"Proxy started on 127.0.0.1:{localProxyPort}. Launching client diverted to local proxy...");
                        // Use native patching to divert the executable to the local listener
                        ClientLauncher.LaunchRedirected(_txtSroPath.Text, targetIp, $"127.0.0.1:{localProxyPort}");
                    }
                    else
                    {
                        LogService.Error($"Failed to connect to original server {targetIp}:{targetPort}");
                        _btnStartGame.Enabled = true;
                    }
        ...
        private void SafeInvoke(Action action)
        {
            ...
            try
            {
                if (this.InvokeRequired)
                {
                    ...
                    {
                        try { action(); }
                        catch (Exception ex) { Console.WriteLine("UI Thread Exp: " + ex.Message); }
                    }));
                }
                else
                {
