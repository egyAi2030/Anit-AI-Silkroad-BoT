...
namespace SilkroadAIBot.Core.Networking
{
    public class PacketHandler
    {
        ...
        private SilkroadAIBot.Core.Data.DataManager _dataManager;
        private List<byte> _characterDataBuffer = new List<byte>();

        public PacketHandler(WorldState worldState, SilkroadAIBot.Core.Data.DataManager dataManager)
        {
        ...
        public void HandlePacket(Packet packet)
        {
            switch (packet.Opcode)
            {
                ...
                case Opcode.SERVER_ENTITY_SPAWN:
                    ParseSingleEntitySpawn(packet);
                    break;
                case Opcode.SERVER_ENTITY_GROUP_SPAWN:
                    ParseGroupSpawn(packet);
                    break;
        ...
        private void ParseItemSpawn(Packet packet, uint refObjID)
        {
            ...
            // ... Pos ...
            Console.WriteLine($"[PacketHandler] Item Spawn: {refObjID} (UID: {uniqueID})");
        }
        ...
        public void ParseMovement(Packet packet)
        {
            try
            {
                ...
                if (entity != null)
                {
                    ...
                    entity.Position = new SRCoord(region, x, y, z);
        ...
        private void ParseBionicDetails(Packet packet, SREntity entity)
        {
            if (entity is SRModel model)
            {
                ...
                model.Position = new SRCoord(region, x, y, z);
                model.Angle = angle;
        ...
        private void ParseSingleEntitySpawn(Packet packet)
        {
            // 0x3015 Single Spawn
            // Structure: ModelID (4) + specialized data based on type
            // Simplified for now
            try
            {
                uint modelID = packet.ReadUInt();
                
                // Need to determine type from ModelID via DataManager
                // For now, assuming Generic Mob/NPC
                
                SRModel entity = new SRModel(); // Base class usage for now
                entity.ModelID = modelID;
                
                ParseBionicDetails(packet, entity);
                
                // Add to WorldState
                _worldState.AddEntity(entity);
                // Console.WriteLine($"[PacketHandler] Spawned Entity {entity.UniqueID} (Model: {modelID})");
            }
            catch (Exception ex)
            {
               // Console.WriteLine($"[PacketHandler] Error parsing Single Spawn: {ex.Message}");
            }
        }

        private void ParseGroupSpawn(Packet packet)
        {
            // 0x3019 Group Spawn
            try
            {
                byte type = packet.ReadByte(); // 1 = Spawn, 2 = Despawn? No, usually count logic
                byte count = packet.ReadByte();
            ...
            {
                // Console.WriteLine($"[PacketHandler] Error parsing Groups Spawn: {ex.Message}");
            }
        }
    }
}
